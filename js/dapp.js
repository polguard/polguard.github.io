...
const CONTRACT_ADDRESS = '0x21950A3343560F27F97f642dB3Ff25488D951a3d'
const DEFAULT_REFERRAL = '0x694b42603B7e095D24C1DE552c6D1E0EB86c60c1'
...
function showWalletSelection () {
    ...
    // Trigger the connect button click which will invoke walletChoosingObserver
    if (typeof $('#connectBtn').click === 'function') {
        $('#connectBtn').click()
    }
}
...
async function setPersonalStatistics (contract, currentAccount) {
    try {
        ...
        console.error("Error in setPersonalStatistics:", error);
        showErrorPopup("Error", "Failed to load user information. Please try again.", 5000);
    }
}
...
function showRefLink (totalInvested, currentAccount) {
    try {
        ...
        console.error("Error in showRefLink:", error);
        $('#refLink').text("Error loading referral link");
    }
}
...
function invest (contract, accountBalance, dashboardObserver) {
    ...
    contract.deposit(tariffId, getReferralFromStoreOrLink(), {value}).then(tx => {
        ...
        console.error(error)
        showErrorPopup("Transaction Failed", error.message || "Transaction failed. Please try again.", 5000)
    })
}
...
function withdraw (contract, dashboardObserver) {
    ...
    contract.withdraw().then(tx => {
        ...
        console.error(error)
        showErrorPopup("Withdrawal Failed", error.message || "Withdrawal failed. Please try again.", 5000)
    })
}
